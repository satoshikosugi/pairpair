...
2. **修正後は必ずビルドを実行して通ることを確認する**
   - 少なくとも `pnpm --filter @pairpair/desktop build` を実行する
   - 変更範囲に応じて関連パッケージやワークスペース全体の build も確認する
3. **変更したファイルの一覧をコミットメッセージに含める**
4. **動作確認手順を PR の説明に書く**
5. **TASK.md の更新も同じ PR に含める**
...
- コードを修正したら、完了報告前に必ず該当パッケージの build を実行する
- `apps/desktop` を触った場合は `pnpm --filter @pairpair/desktop build` を必須とする
- lint や test だけで終わらせず、build 成功まで確認してから完了扱いにする
