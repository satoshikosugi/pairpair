...
- `src/renderer/routes/SessionPage.tsx`
  ...
  - ゲスト: 画面表示、マーカー、`Ctrl + 右クリック` での注目スポット送信、スニペット送信、全画面、ESC 2回で復帰
- `src/renderer/components/RemoteVideoView.tsx`
  - 映像表示、入力送信、注釈描画、ホバー位置送信、`Ctrl + 右クリック` での注目スポット送信、注目スポット表示
  - 等倍表示時の中央寄せと右ドラッグパン
...
- `src/renderer/components/PermissionPanel.tsx`
  - 権限プリセット、クリップボード送信、送受信履歴、ローカル受け取り
...
- `src/rtc-types.ts`
  ...
  - 注釈同期、ゲストカーソル投影、注目スポット同期、スニペット履歴同期を追加
- `src/input-events.ts`
  - 入力イベント型
- `src/annotation.ts`
...
- 同一 repo 内コードとしては、接続、共有、入力、品質変更、注釈、注目スポット、クリップボード送受信まで一通り実装済み
